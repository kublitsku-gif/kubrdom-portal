...
export function isoScene(model, winTypes, opts) {
  ...
  sc.walls.forEach(function (w) {
    ...
    // На плане торцевые стены чертят между продольными — так принято, и угол
    // получается с перевязкой. В объёме из-за этого торец на 76 мм уже габарита, и
    // крыша, идущая по габариту, вылезает на углу зубцом. Достраиваем торец до
    // габарита: это тот же угол, только видимый снаружи, а не в разрезе.
    const box = (shell && !along)
      ? { x: w.x, y: 0, w: w.w, h: sc.w }
      : { x: w.x, y: w.y, w: w.w, h: w.h };

    boxFaces(project, box, 0, H, kind, function (n) {
      ...
      // У целого дома отвёрнутые грани оставляем: их всё равно закроет своя же
      // стена, зато силуэт коробки становится ПОЛНЫМ. Без них дальняя стена
      // кончалась на своей внутренней плоскости, и крыша свисала над ней на
      // толщину стены — это и читалось как зазор в кровле.
      if (mode === "solid" && shell) return false;

...
// Редактируемая в портале инструкция (settings.aiSellerPrompt) или дефолт. Worker читает её из D1.
async function aiSellerPrompt(env) {
  try {
    ...
    if (row && row.data) { const s = JSON.parse(row.data); if (s && typeof s.aiSellerPrompt === "string" && s.aiSellerPrompt.trim()) return s.aiSellerPrompt.slice(0, 8000); }
  } catch (e) {}
...
async function aiSellerReply(env, history) {
  const sys = await aiSellerPrompt(env);
  const messages = [

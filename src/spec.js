...
// Сколько ЦЕЛЫХ покупок нужно на площадь — подсказка у дописанного руками
// материала («Стены спальня · 26,87 м²» → 12 листов фанеры). Расчётный считается
// сам, а дописанный встаёт числом из формы, и без подсказки человек делит в уме.
// 0 — подсказать нечего: штучный товар, фасовка не задана или площади у строки нет.
export function matNeedForArea(mat, area) {
  const a = Number(area) || 0;
  if (!mat || a <= 0) return 0;
  const mode = mat.mode || "piece";
  if (mode === "m2") return Math.round(a * 100) / 100;
  const per = Number(mat.packPer) || 0;
  if ((mode !== "pack" && mode !== "sheet") || mat.packBase !== "м²" || per <= 0) return 0;
  // 4,65 / 2,325 в плавающей точке — 2,0000000000000004: без округления до
  // миллионных «ровно два листа» превращались в три.
  return Math.ceil(Math.round(a / per * 1e6) / 1e6);
}

// Совпадает ли количество с объёмом строки. Расчётный лист считается дробно
// (8,96 листа ГКЛ на 26,87 м²), а купить можно только целый — это те же 9
// покупок, и зажигать на такой строке «нужно 9» значило бы спорить с расчётом.
export function matNeedOk(mat, area, qty) {
  const need = matNeedForArea(mat, area);
  if (!(need > 0)) return false;
  const q = Number(qty) || 0;
  if ((mat.mode || "piece") === "m2") return Math.abs(q - need) < 0.01;
  return Math.ceil(Math.round(q * 1e6) / 1e6) === need;
}

// С каким объёмом дома сходится количество (`vols` — [{ n, area }]). В строке
// «Монтаж стен из ОСП» звукоизоляция идёт только в перегородки: её 2 пачки не
// сходятся со стенами дома, но сходятся с перегородками — и зажигать на ней
// «нужно 16» значило бы подсказывать ошибку. null — ни с чем не сходится.
export function matNeedMatch(mat, qty, vols) {
  const list = vols || [];
  for (let i = 0; i < list.length; i++) {
    if (list[i] && matNeedOk(mat, list[i].area, qty)) return list[i];
  }
  return null;
}

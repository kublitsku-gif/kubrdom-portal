...
//
// Труба стоит СНАРУЖИ листа и варится к нему с обеих сторон. Наружу она уходит на
// 60 мм, а по фасаду занимает 40 — и в разметку идёт именно 40: только эта грань
// лежит в плоскости стены. Вылет 60 в ширину реза не входит вообще, он живёт
// поперёк листа, и складывать его с шириной проёма значит резать лишнее.
export const JAMB_TUBE = 40;          // сторона трубы ПО ФАСАДУ, мм
export const JAMB_TUBE_OUT = 60;      // вылет трубы НАРУЖУ от листа, мм
...
export function modelScheme(model, winTypes) {
  ...
  // Трубы стоят за зазором от изделия и варятся СНАРУЖИ ЛИСТА контейнера, а не
  ...
  // плюс два зазора. На плане это два прямоугольника 40 вдоль стены на 60 наружу —
  // по ним размечают стойки, и лежат они ЗА обводом коробки, а не в её толще:
  // труба снаружи, и рисовать её внутри стены значило бы врать о том, где варить.
  const jambsOf = function (box, along, outw) {
    const t2 = JAMB_TUBE, g = JAMB_GAP, d = JAMB_TUBE_OUT;
    ...
    if (along) {
      const face = (n[1] > 0) ? (box.y + box.h) : box.y;
      const y = (n[1] > 0) ? face : face - d;
      return [{ x: box.x - g - t2, y: y, w: t2, h: d },
              { x: box.x + box.w + g, y: y, w: t2, h: d }];
    }
    const face = (n[0] > 0) ? (box.x + box.w) : box.x;
    const x = (n[0] > 0) ? face : face - d;
    return [{ x: x, y: box.y - g - t2, w: d, h: t2 },
            { x: x, y: box.y + box.h + g, w: d, h: t2 }];
  };
  ...
  const openings = (m.openings || []).map(function (op) {
    ...
    return {
      ...
      // Ширина реза в стене: изделие плюс два монтажных зазора. Трубы стоят СНАРУЖИ
  ...
  // уже реза на два монтажных зазора. Труба варится СНАРУЖИ листа, её внутренняя

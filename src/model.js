...
// Усиление проёма: по обе стороны окна или входной двери стоит труба 60×40 во всю
...
//
// Труба стоит В САМОМ РЕЗЕ и центрируется по листу: 60 мм идут поперёк стены, по
// 30 наружу и внутрь, а лист упирается в её грань торцом и варится с обеих сторон.
// Поэтому рез ШИРЕ проёма между трубами на две трубы: сначала вырезают дырку под
// обрамление, и только потом в неё встаёт окно. Считать рез по изделию плюс зазоры
// (как когда труба лежала накладкой снаружи) значит вырезать на 80 мм меньше — и
// трубу в такой рез уже не вставить.
export const JAMB_TUBE = 40;          // сторона трубы ВДОЛЬ стены, мм
export const JAMB_TUBE_CROSS = 60;    // сторона трубы ПОПЕРЁК стены, мм (по 30 наружу и внутрь)
...
export const MODEL_PRESETS = [
  ...
  {
    k: "mordves-1",
    n: "Мордвес 1 · санузел / зал / спальня",
    note: "По чертежу 1200 × 247 см: 4,40 + 13,61 + 7,46 м²",
    // Отличие от «Дома для СВО» — вход и окно зала в ВЕРХНЕЙ стене, а не в нижней,
    // и отсеки другие: 200 · 620 · 340 в чистоте.
    type: "40hc", l: 11952, w: 2352, h: 2500, wallThick: WALL_THICK, finish: FINISH_THICK,
    rooms: [
      { name: "Санузел", len: 2076, pts: {} },   // 2000 в чистоте + отделка торца
      { name: "Зал",     len: 6200, pts: {} },
      { name: "Спальня", len: 3476, pts: {} },   // 3400 + отделка торца
    ],
    needs: [
      { cat: "os-vd-1000x2100" },    // вход 1000×2100, H=210
      { cat: "os-1000x2100" },       // окно зала 1000×2100
      { cat: "os-1850x2100" },       // торцевое окно спальни, на чертеже 2000×2200
      { cat: "inner-600x2050" },
    ],
    // Отметки — по верхней цепочке чертежа: 342 · 100 (вход) · 178 · 100 (окно) · 479.
    openings: [
      { side: "n", pos: 3420, sill: 0,   need: 0 },
      { side: "n", pos: 6200, sill: 0,   need: 1 },
      // Торцевое окно спальни: Н под.=20 с чертежа, по центру торца.
      { side: "e", pos: 251,  sill: 200, need: 2 },
      // Дверь санузла: проём 700 по центру (75 · 70 · 75), открывается в зал.
      { side: "part", afterRoom: 0, pos: 876, need: 3, into: 1,  hinge: "start" },
      // Дверь спальни у верхней стены — под ней на чертеже шкаф; тоже в зал.
      { side: "part", afterRoom: 1, pos: 126, need: 3, into: -1, hinge: "start" },
    ],
  },
  {
    k: "svo",
    ...
    // Изделия — ключами каталога. Окно зала — то самое №3 из спецификации:
    // 1500×1200, глухое поле 750 плюс поворотно-откидная створка 750 с петлями
    // слева. Ширина сходится с чертежом в ноль, подоконник выходит 900 — обычная
    // отметка для комнаты, где под окном стоит мебель.
    needs: [
      { cat: "os-1500x1200" },       // окно зала, п/о створка
      { cat: "os-vd-1000x2100" },    // вход 1000×2100, H=210
      { cat: "os-1850x2100" },       // торцевое окно спальни, на чертеже 2000×2200
      { cat: "inner-600x2050" },
    ],
    ...
    openings: [
      { side: "s", pos: 2976, sill: 900, need: 0 },   // окно зала: 800 от стены санузла
      { side: "s", pos: 5276, sill: 0,   need: 1 },   // вход, 800 от окна
      // Торцевое окно спальни: Н под.=20 с чертежа, по центру торца.
      { side: "e", pos: 251,  sill: 200, need: 2 },
...
export function modelScheme(model, winTypes) {
  ...
  // Трубы стоят за зазором от изделия и вставляются В САМ РЕЗ, поперёк стены: 60 мм
  // делятся пополам наружной гранью листа, по 30 в каждую сторону. На плане это два
  // прямоугольника 40 вдоль стены на 60 поперёк, ОСЕДЛАВШИЕ линию наружной стены, —
  // по ним размечают стойки. Нарисованные целиком снаружи или целиком в толще, они
  // сказали бы бригаде не ту глубину посадки.
  const jambsOf = function (box, along, outw) {
    const t2 = JAMB_TUBE, g = JAMB_GAP, d = JAMB_TUBE_CROSS;
    // Нормаль наружу известна только у наружных стен, но усиление больше нигде и
    // не ставится (`frameNeeded`). Запасной вариант нужен на случай, если проём
    // сделают с усилением руками: лучше нарисовать трубу с одной стороны, чем упасть.
    const n = outw || (along ? [0, -1] : [-1, 0]);
    if (along) {
      const face = (n[1] > 0) ? (box.y + box.h) : box.y;   // наружная грань листа
      return [{ x: box.x - g - t2, y: face - d / 2, w: t2, h: d },
              { x: box.x + box.w + g, y: face - d / 2, w: t2, h: d }];
    }
    const face = (n[0] > 0) ? (box.x + box.w) : box.x;
    return [{ x: face - d / 2, y: box.y - g - t2, w: d, h: t2 },
            { x: face - d / 2, y: box.y + box.h + g, w: d, h: t2 }];
  };
  ...
  const openings = (m.openings || []).map(function (op) {
    ...
    let box = null, out = null, along = true, outward = null;
    if (op.side === "part") {
      ...
      outward = (op.side === "n") ? [0, -1] : (op.side === "s") ? [0, 1] : (op.side === "w") ? [-1, 0] : [1, 0];
    ...
    return {
      ...
      frame: frame, jambs: frame ? jambsOf(box, along, outward) : [],
      // Ширина реза в стене: изделие, два монтажных зазора И ДВЕ ТРУБЫ. Трубы стоят
      // в самом резе, поэтому входят в него: сначала режут дырку под обрамление,
      // потом в неё встаёт окно. По этому числу режут, и оно обязано приезжать из
      // одного места — сюда смотрят и чертёж, и таблица на печатном листе.
      cutW: wd + (frame ? 2 * (JAMB_GAP + JAMB_TUBE) : 0),
  ...
  // уже реза на две трубы и два монтажных зазора. Труба стоит В резе (см. узел 1),
  // поэтому она в этот размер входит: рез = проём между трубами + две трубы.
  const cut = function (o) {
    const pad = o.frame ? (JAMB_GAP + JAMB_TUBE) : 0;

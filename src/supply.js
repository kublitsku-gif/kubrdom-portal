...
// `m.client` не выводит её из закупки, а помечает, что за эти деньги платит
// заказчик: по такой пометке собирается счёт на оплату материалов.
//
...
// Договорённость про оплату — свойство ОБЪЕКТА (`o.clientPays`), а не всего
...
// Что идёт в счёт заказчику: помеченное насовсем плюс добранное разово
...
// закупили и за что можно просить деньги прямо сейчас; остальное ещё впереди.

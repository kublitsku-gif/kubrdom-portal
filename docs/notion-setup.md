...
> **Важно про размер (зависит от плана Notion):**
> - **Free-план:** только `single_part`, максимум **5 МБ на файл**. Multipart Notion
>   на бесплатном плане запрещает вовсе (ответ: *«This workspace is on a free plan and
>   does not support multipart uploads»*). Т.е. по-настоящему большие файлы бесплатный
>   воркспейс не примет.
> - **Платный воркспейс:** до **5 ГБ на файл**, multi-part включается автоматически при >20 МБ.
>
> Код это учитывает и переключается сам — при апгрейде плана менять ничего не нужно.
> Текущий лимит своего воркспейса видно в `GET /v1/users/me` → `bot.workspace_limits.max_file_upload_size_in_bytes`.

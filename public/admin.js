...
function _decodeToken(t){
  try{
    if(typeof t!=="string"||t.indexOf("v1.")!==0)return null;
...
let clientLoginMatch=null;    // id найденного договора (legacy, поиск теперь на сервере)
let clientLoginQuery="";      // введённый клиентом номер/фамилия — уходит в /api/client-login
...
function estRoom(e, kind){
  // Сохранённую комнату используем, только если она из набора текущего типа (баня/дом).
  // Иначе (напр. дом-смета скопирована из бани с банными ключами) — определяем по названию,
  // чтобы работа не выпала из группировки.
  if(e&&e.room&&roomsFor(kind).some(function(rm){return rm.k===e.room;}))return e.room;
...
function breakdownNote(bd){
  ...
  const parts=bd.filter(function(r){return (Number(r.n)||0)>0;}).map(function(r){return numRu(Number(r.n)||0)+"×"+numRu(Number(r.len)||0)+" м";});
  if(!parts.length)return "";
  return parts.join(" + ")+" = "+numRu(breakdownTotal(bd))+" м.п.";
}
function lenSelectOptions(cur){
  const c=Number(cur)||0, has=LEN_OPTIONS.indexOf(c)>=0;
  let o=LEN_OPTIONS.map(function(L){return '<option value="'+L+'"'+(L===c?' selected':'')+'>'+numRu(L)+' м</option>';}).join("");
  if(!has&&c>0)o='<option value="'+c+'" selected>'+numRu(c)+' м</option>'+o;
  return o;
}
...
function loginPage(){
  ...
  if(loginMode==="client"){
    // Шаг 2 — ввод PIN (договор ищется и сверяется на сервере в /api/client-login).
    if(clientLoginStep==="pin"){
      const inner='<div style="background:#fff;border-radius:20px;padding:24px;box-shadow:0 4px 24px rgba(0,0,0,0.08);border:1px solid #e8eef5;text-align:center">'+
          ...
          '<div style="font-size:15px;font-weight:700;color:#0d1b2e">Вход в кабинет</div>'+
          '<div style="font-size:12px;color:#7a9aaa;margin-bottom:16px">'+esc(clientLoginQuery||"")+'</div>'+
          ...
          (clientLoginError?'<div style="font-size:12px;color:#e74c3c;font-weight:600;margin-top:8px">'+esc(clientLoginError)+'</div>':'')+
...
function bindLogin(){
  ...
  // Клиент: ввод номера/фамилии — поиск теперь на сервере (в /api/client-login), тут только сохраняем запрос.
  document.querySelectorAll("[data-a='client-find']").forEach(function(el){
    const go=function(){
      const inp=document.getElementById("client-query");
      const q=(inp?inp.value:"").trim();
      if(!q){ clientLoginError="Введите номер договора или фамилию."; render(); return; }
      clientLoginQuery=q; clientLoginStep="pin"; clientLoginError="";
      render();
    };
    el.onclick=go;
  });
  ...
  // Клиент: проверка PIN на сервере (/api/client-login) и вход в кабинет со срезом своего договора.
  document.querySelectorAll("[data-a='client-pin-submit']").forEach(function(el){
    const go=async function(){
      ...
      if(!val){ clientLoginError="Введите PIN"; render(); return; }
      clientLoginError="Проверяем…"; render();
      try{
        const r=await fetch(API_BASE+"/api/client-login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:clientLoginQuery,pin:val})});
        const j=await r.json();
        if(!j||!j.success){ clientLoginError=(j&&j.error)||"Ошибка входа"; render(); return; }
        setToken(j.token);
        if(Array.isArray(j.items)) applyState(j.items);
        clientAuthContract=j.cid;
        _hydrated=true;
        loginMode=null; clientLoginStep="find"; clientLoginMatch=null; clientLoginQuery=""; clientLoginError=""; clientTab="objects";
        render();
      }catch(e){ clientLoginError="Нет связи с сервером."; render(); }
    };
    el.onclick=go;
  });
  ...
  document.querySelectorAll("[data-a='emp-login-go']").forEach(function(el){
    const go=async function(){
      ...
      if(!pin){ empPhoneError="Введите PIN"; render(); return; }
      const rm=document.getElementById("remember-me"); const remember=(!rm||rm.checked);
      empPhoneError="Проверяем…"; render();
      try{
        const r=await fetch(API_BASE+"/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:phone,pin:pin})});
        const j=await r.json();
        if(!j||!j.success){ empPhoneError=(j&&j.error)||"Ошибка входа"; render(); return; }
        setToken(j.token);
        ...
        _startLoops(); _setInitialTab(); render();
      }catch(e){ empPhoneError="Нет связи с сервером. Проверьте интернет или включите VPN."; render(); }
    };
    el.onclick=go;
  });
  ...
  // Проверка PIN и вход — на сервере (/api/login по userId выбранного сотрудника).
  document.querySelectorAll("[data-a='login-pin-submit']").forEach(function(el){
    const doLogin=async function(){
      const uid=loginPinFor;
      const inp=document.getElementById("login-pin");
      const val=(inp?inp.value:"").trim();
      if(!val){ loginPinError="Введите PIN"; render(); return; }
      const rm=document.getElementById("remember-me"); const remember=(!rm||rm.checked);
      loginPinError="Проверяем…"; render();
      try{
        const r=await fetch(API_BASE+"/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:uid,pin:val})});
        const j=await r.json();
        if(!j||!j.success){ loginPinError=(j&&j.error)||"Ошибка входа"; render(); return; }
        setToken(j.token);
        try{ if(remember) localStorage.setItem("kubr_remember",j.user.id); else localStorage.removeItem("kubr_remember"); }catch(e){}
        ...
        _startLoops(); _setInitialTab(); render();
      }catch(e){ loginPinError="Нет связи с сервером."; render(); }
    };
    el.onclick=doLogin;
  });
  // Enter в поле PIN
...
function render(){
  ...
  if(clientAuthContract){
    ...
    if(lo)lo.onclick=function(){ clearToken(); clientAuthContract=null; loginMode=null; clientTab="objects"; render(); };
        ...
        ${(tw.mats||[]).length?(tw.mats||[]).map(m=>{const mo=EXP_MODES.find(x=>x.k===(m.mode||"piece"))||EXP_MODES[0];const conv=expConv({mode:m.mode,unitCost:Number(m.cost)||0,packBase:m.packBase,packPer:m.packPer,sheetM2:m.sheetM2,lenPer:m.lenPer});const lt=Math.round((Number(m.cost)||0)*(m.qty||0));
          ...
          return`<div style="padding:6px 0;border-top:1px solid #f0f4f8">
            ...
            ${_showBd?`<div style="margin-top:6px;background:#fafbfc;border:1px dashed ${_bd.length?'#16a085':'#cdd8e6'};border-radius:8px;padding:7px 9px">
              <div style="font-size:9px;font-weight:700;color:#7a9aaa;letter-spacing:0.3px;margin-bottom:5px">📐 РАЗБИВКА ПО ХЛЫСТАМ</div>
              ${_bd.map((r,ri)=>`<div style="display:flex;gap:5px;align-items:center;margin-bottom:4px">
                <select data-a="bd-len" data-eid="${e.id}" data-mid="${m.id}" data-ri="${ri}" style="padding:5px 4px;border-radius:6px;border:1px solid #d0dae8;font-size:11px;outline:none;background:#fff;cursor:pointer">${lenSelectOptions(r.len)}</select>
                <span style="font-size:11px;color:#9aabbf">×</span>
                <input data-a="bd-n" data-eid="${e.id}" data-mid="${m.id}" data-ri="${ri}" type="number" step="any" value="${r.n}" style="width:48px;padding:5px 4px;border-radius:6px;border:1px solid #d0dae8;font-size:11px;text-align:center;outline:none">
                <span style="font-size:11px;color:#9aabbf">шт</span>
                <span style="flex:1;text-align:right;font-size:11px;font-weight:700;color:#5a7a9a">${numRu(Math.round((Number(r.len)||0)*(Number(r.n)||0)*100)/100)} ${_unit}</span>
                <button data-a="bd-del" data-eid="${e.id}" data-mid="${m.id}" data-ri="${ri}" title="Удалить" style="width:22px;height:22px;background:transparent;border:1px solid #e74c3c44;border-radius:5px;cursor:pointer;color:#e74c3c;font-size:10px;flex-shrink:0">✕</button>
              </div>`).join('')}
...
function tVoiceAi(){
  ...
  html+='</div>';
  // Результаты (с сервера: work_id=voiceCalls)
  const calls=(voiceCallsCache||[]).slice().reverse();
  html+='<div style="background:#fff;border:1px solid #dde6f0;border-radius:12px;padding:12px 14px;margin-bottom:12px">';
  html+='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><div style="font-size:12px;font-weight:800;color:#1a2a3a">📋 Результаты звонков</div><button data-a="va-refresh" style="font-size:11px;font-weight:700;color:#2980b9;background:#eef4fb;border:1px solid #2980b933;border-radius:7px;padding:4px 9px;cursor:pointer">🔄 Обновить</button></div>';
  if(!calls.length){
...
// ИП Кублицкой). PDF получается через диалог печати браузера (Печать → Сохранить как PDF).
...
// ═══ Редактируемый текст договора ═══
// Дефолтные разделы (общие для бани и дома) — plain text с плейсхолдерами.
// Правки хранятся в синхронизируемом settings.ctTplTexts[banya|house] (отдельно на тип),
// отсутствие записи = стандартный текст. Плейсхолдеры подставляет генератор:
// {ПРЕДМЕТ} {СУММА} {СУММА_ПРОПИСЬЮ} {ПЛАТЕЖИ} {СРОК} {СРОК_ПРОПИСЬЮ}
// {ГАРАНТИЯ} {ГАРАНТИЯ_ПРОПИСЬЮ} {ГАРАНТИЯ_ЕД} {НОМЕР} {ИСПОЛНИТЕЛЬ} {ЗАКАЗЧИК}
// Разметка: строка = абзац; строки, начинающиеся с «- », собираются в маркированный список;
// ведущий номер пункта (1.1., 2.1.4. …) автоматически выделяется жирным.
const CT_DOC_SECTIONS=[
{t:"1. Предмет Договора",b:`1.1. По настоящему Договору Заказчик поручает, а Исполнитель обязуется выполнить работы по изготовлению {ПРЕДМЕТ}:
- с планировкой (Приложение № 1 к настоящему Договору),
- графиком платежей (Приложение № 2 к настоящему Договору),
- спецификацией (Приложение № 3 к настоящему Договору),
- актом приема-передачи (Приложение № 4 к настоящему Договору)
и условиями настоящего Договора. Перечень выполняемых Работ определяется Спецификацией, являющейся неотъемлемой частью Договора. По согласованию Сторон Приложения могут корректироваться в переписке телеграмм или макс на телефонных номерах, указанных в договоре.
1.2. Заказчик обязуется принять и оплатить выполненные Исполнителем работы по предмету настоящего Договора на условиях настоящего Договора.`},
{t:"2. Права и обязанности Сторон",b:`2.1. Исполнитель обязан:
2.1.1. Качественно и в надлежащие сроки выполнить своим иждивением и сдать Заказчику работы по предмету настоящего Договора в соответствии и на условиях настоящего Договора.
2.1.2. Осуществить в надлежащие сроки доставку, разгрузку и установку объекта на участок Заказчика за счет средств Заказчика. Точная сумма определяется перед отгрузкой.
2.1.3. Осуществлять контроль и надзор за ходом и качеством выполняемых работ по предмету настоящего Договора. Предоставить Заказчику возможность осуществлять проверку этапов выполнения и качества работ по Договору.
2.1.4. Уведомлять Заказчика об окончании, продлении или приостановке работ по предмету настоящего Договора, а также о любых проблемах, возникающих в ходе исполнения обязательств по предмету настоящего Договора, любым из перечисленных ниже способов в одностороннем порядке: текстовое сообщение или телефонный звонок на номера телефонов, указанные в п. 12 настоящего Договора, либо сообщение на адрес электронной почты, указанный в п. 12 настоящего Договора.
2.2. Исполнитель имеет право:
2.2.1. По согласованию Сторон заменять используемые при выполнении работ по предмету настоящего Договора Материалы на аналогичные по свойствам и качеству.
2.2.2. Размещать, на время строительства, по своему усмотрению рекламные баннеры на Строительной площадке (забор, здания).
2.2.3. При необходимости привлекать к выполнению работ по предмету настоящего Договора третьих лиц.
2.2.4. Использовать фотографии возведенного Объекта в печатных и интернет-изданиях.
2.2.5. Исполнитель вправе извещать Заказчика о завершении любого промежуточного этапа работы и о завершении всех (итоговых) работ по договору путем направления Заказчику фотоснимков результатов соответствующего этапа работ посредством электронной связи через мессенджеры MAX или Telegram, что имеет юридическую силу надлежащего уведомления Исполнителем Заказчика об окончании выполнения промежуточного или итогового этапа работ по договору и необходимости его приемки Заказчиком. Подписывая настоящий договор, Заказчик выражает свое согласие с таким способом направления уведомления о готовности работ к приемке. Уведомления и иные сообщения, направленные одной Стороной, считаются доставленными и полученными другой Стороной по истечении одного календарного дня с момента его отправки.
2.3. Заказчик обязан:
2.3.1. Принять и оплатить на условиях настоящего Договора выполненные работы по предмету настоящего Договора (законченный Объект), а также подписать Акт приема-передачи выполненных работ (Приложение № 4 к настоящему Договору).
2.3.2. Надлежаще оформить необходимые для такого вида работ дополнительные документы (в том числе бухгалтерские) на момент подписания Сторонами Акта приема-передачи выполненных работ (Приложение № 4 к настоящему Договору) в соответствии с условиями настоящего Договора.
2.3.3. Принимать каждый этап выполненных работ по предмету настоящего Договора, приезжая на производственную площадку либо через фото или видео отчет, подписывать промежуточные акты приема-передачи выполненных работ и оплачивать их в соответствии с Графиком платежей (Приложение № 2 к настоящему Договору).
2.4. Заказчик имеет право:
2.4.1. Осуществлять надзор за ходом, качеством, сроками проведения работ по предмету настоящего Договора как лично, так и через своего уполномоченного представителя, предварительно известив об этом Исполнителя и согласовав с Исполнителем время посещения Строительной площадки;
2.4.2. Приостанавливать работы по предмету настоящего Договора в случае несогласия с ходом или качеством таких работ на основании заявления, составленного в простой письменной форме.`},
{t:"3. Цена и порядок расчетов",b:`3.1. По согласованию Сторон цена работ по предмету настоящего Договора составляет: {СУММА} ({СУММА_ПРОПИСЬЮ}).
3.2. По согласованию Сторон расчет по настоящему Договору производится в соответствии с Графиком платежей (Приложение № 2 к настоящему Договору) в валюте РФ (рубли) способом, не противоречащим действующему законодательству РФ, следующим образом:
{ПЛАТЕЖИ}
3.3. Моментом оплаты считается дата зачисления денежных средств на расчетный счет Исполнителя, указанный в п. 12 настоящего Договора, либо дата внесения Заказчиком денежных средств в кассу Исполнителя, либо дата получения расписки в получении денежных средств.
3.4. С письменного согласия Сторон цена выполненных Исполнителем работ по предмету настоящего Договора может быть изменена на основании переписки в телеграмм или макс на телефонных номерах, указанных в договоре.
3.5. ВАЖНО! В стоимость работ не входят расходы по доставке, разгрузке и установке объекта на участке.`},
{t:"4. Срок исполнения Договора",b:`4.1. Настоящий Договор вступает в законную силу с момента его подписания Сторонами и действует до полного исполнения Сторонами принятых на себя обязательств.
4.2. Срок выполнения работ по предмету настоящего Договора определен Исполнителем в {СРОК} ({СРОК_ПРОПИСЬЮ}) рабочих дней с момента получения первого аванса по настоящему договору подряда, указанному в п. 3.2.1.
4.3. В случае нарушения Заказчиком сроков платежей денежных средств Исполнителю, указанных в п. 3.2 настоящего Договора, срок исполнения условий настоящего Договора увеличивается на количество календарных дней просрочки Заказчиком каждого из платежей.
4.4. По согласованию Сторон срок исполнения условий настоящего Договора Сторонами (отдельно каждого из этапов, указанных в п. 3.2 настоящего Договора) может корректироваться на основании надлежаще оформленных Дополнительных соглашений, являющихся неотъемлемой частью настоящего Договора.
4.5. В случае приостановки работ по предмету настоящего Договора по инициативе Заказчика для исполнения последним условий п. 2.4.2 настоящего Договора, Исполнитель вправе продлить в одностороннем порядке срок исполнения условий настоящего Договора, указанный в п. 4.2 настоящего Договора, на количество календарных дней приостановки работ по предмету настоящего Договора и потребовать от Заказчика возмещение ущерба в результате вынужденного простоя, исходя из расчета 15 000 (пятнадцать тысяч) рублей в день за каждый день простоя. В случае нарушения Исполнителем сроков строительства предусмотрено возмещение 0,1% в день от оставшейся суммы платежей, но не более 5% от суммы всего договора.
4.6. По согласованию Сторон срок исполнения условий настоящего Договора может быть увеличен, в том числе, по причине невозможности соблюдения сроков по настоящему Договору в результате ухудшения погодных условий или непредвиденных обстоятельствах, но не более чем на 30 (тридцать) календарных дней. Исполнитель уведомляет Заказчика об увеличении срока исполнения условий настоящего Договора любым из способов, указанных п. 2.1.4 настоящего Договора.`},
{t:"5. Гарантийные обязательства",b:`5.1. Исполнитель принимает на себя гарантийные обязательства на Объект на {ГАРАНТИЯ} ({ГАРАНТИЯ_ПРОПИСЬЮ}) {ГАРАНТИЯ_ЕД} с момента подписания Сторонами Акта приема-передачи выполненных работ.
5.2. Гарантийные обязательства не распространяются на следующие свойства Материалов из древесины: усадка, усушка, разбухание, появление трещин, появление синевы и деформации вследствие воздействия влажной среды, так как при изготовлении таких Материалов нарушается природная структура древесины.
5.3. Гарантийные обязательства также не распространяются:
- на фундаменты, изготовленные Заказчиком (временные, столбчатые, поверхностные, ленточные, из винтовых свай и т.п.);
- на нарушение целостности конструкции Объекта либо отдельных его частей вследствие разрушения, наклона, обвала или просадки изготовленного Заказчиком фундамента;
- на ущерб, нанесенный Объекту третьими лицами, либо Заказчиком (его представителями), вследствие одностороннего изменения последними технического задания, проекта; а также на ущерб, возникший в результате эксплуатации не в соответствии с существующими нормами;
- на повреждения любых элементов Объекта, в случае, если Заказчик в течение срока действия гарантийных обязательств и во время выполнения работ по предмету настоящего Договора изменяет конструкцию или технико-эксплуатационные параметры Объекта без согласования с Исполнителем;
- на материалы, используемые при проведении работ по предмету настоящего Договора, предоставленные Заказчиком;
- несоблюдение температурно-влажностного режима, несоблюдение норм эксплуатации и мер безопасности;
- на усушку внутренней отделки, и как следствие — в гарантийные обязательства не входит перетяжка отделки;
- на внесение изменений в конструктив, не согласованный с Исполнителем и не заверенный Актом. К подобным изменениям относится перенос перегородок, перенос окон, изменение «пирога» стены, изменение внешнего вида и любые отклонения от технического задания и проекта, указанного в договоре;
- на окна, двери, мебель, сантехнику, инженерное оборудование распространяются гарантийные обязательства производителей.
5.4. Гарантийные обязательства распространяются на протекание кровли и целостность конструкции Объекта.
5.5. После подписания Сторонами Акта приема-передачи выполненных работ претензии по качеству Объекта принимаются Исполнителем только в рамках гарантийных обязательств.
5.6. Исполнитель в течение гарантийного срока за свой счет и своими силами устраняет выявленные в процессе эксплуатации Объекта недостатки в рамках гарантийных обязательств.
5.7. Выявленные Заказчиком в рамках гарантийных обязательств недостатки Объекта в течение гарантийного срока оформляются соответствующими Актами, подписанными Сторонами. Без оформления упомянутых выше Актов претензии Заказчика по недостаткам Объекта не подлежат удовлетворению.
5.8. Стороны согласовали, что ввиду того, что морской контейнер является бывшим в употреблении, на фасадной (внешней части) Объекта могут быть незначительные вмятины и иные дефекты, не влияющие на качественные характеристики Объекта. Также допустимы незначительные повреждения внутренней и внешней отделки контейнера, вызванные транспортировкой (мелкие трещины, локальное отклеивание плитки, незначительное изменение расстояния между хлыстами МДФ, незначительная деформация рам и дверей). Подписывая настоящий Договор, Заказчик соглашается с тем, что наличие вышеуказанных дефектов не является основанием для отказа от приемки Объекта.`},
{t:"6. Ответственность Сторон",b:`6.1. В случае нарушения срока исполнения условий, указанных в п. 4.2 настоящего Договора, Исполнитель уплачивает Заказчику пени в размере 0,1 % от цены, указанной в п. 3.1 настоящего Договора, за каждый день просрочки, но всего не более 5 (пяти) % от суммы договора.
6.2. В случае нарушения сроков оплаты, указанных в п. 3.2 настоящего Договора, Заказчик уплачивает Исполнителю пени в размере 0,1 % от цены каждого неоплаченного этапа работ по предмету настоящего Договора за каждый день просрочки, но всего не более 5 (пяти) % от суммы договора, помимо санкций, указанных в других пунктах настоящего Договора.
6.3. В случае неявки уполномоченных представителей Заказчика на приемку выполненных скрытых работ по предмету настоящего Договора в срок, такие работы считаются принятыми и подлежат оплате. Вскрытие Исполнителем выполненных скрытых работ по предмету настоящего Договора считается «Дополнительными работами» и подлежит оплате Заказчиком на основании надлежаще оформленных Дополнительных соглашений, являющихся неотъемлемой частью настоящего Договора.
6.4. В случае, если Заказчик отказывается принимать выполненные работы по предмету настоящего Договора, в том числе, по отдельным этапам таких работ, последний обязан в трехдневный срок с момента отказа, в письменной форме предоставить Исполнителю мотивированные (документально подтвержденные) причины отказа. В противном случае работы по предмету настоящего Договора считаются выполненными, принимаются в полном объеме и подлежат оплате Заказчиком. Без документального подтверждения отказ не является обоснованным.
6.5. За необоснованный отказ Заказчика от приемки выполненных работ по предмету настоящего Договора, Исполнитель имеет право взыскать с Заказчика неустойку в размере 25 (двадцати пяти) % от цены, указанной в п. 3.1 настоящего Договора, что не освобождает Заказчика от полного исполнения условий, указанных в п. 3.2 и п. 6.2 настоящего Договора.
6.6. Применение к ответственной Стороне мер имущественной ответственности не отменяет ее обязанности исполнить иные обязательства по настоящему Договору.
6.7. Стороны пришли к согласию о том, что указанные в настоящем Договоре пределы имущественной ответственности (штрафные санкции) начисляются с момента их письменного признания Сторонами в бесспорном порядке, либо на основании судебного решения. До этого, исключительно в целях бухгалтерского и налогового учетов, пределы имущественной ответственности (штрафные санкции) составляют 0 %.
6.8. В случае, если Заказчик расторгает настоящий Договор в одностороннем порядке до начала работ по предмету настоящего Договора, то в зависимости от количества рабочих дней, прошедших с момента совершения Заказчиком предварительной оплаты в соответствии с условиями п. 3.2 настоящего Договора до момента расторжения настоящего Договора Заказчиком:
- если от 1-го до 5-ти рабочих дней — Заказчику возвращается 100% суммы указанной выше предварительной оплаты;
- если от 5-ти до 13-ти рабочих дней — Заказчику возвращается 50% суммы указанной выше предварительной оплаты;
- если от 14-ти и более рабочих дней — вся сумма указанной выше предварительной оплаты остается у Исполнителя.
6.9. В случае нарушения Заказчиком сроков оплаты, указанных в п. 3.2 настоящего Договора, Исполнитель вправе приостанавливать работы по предмету настоящего Договора до полного исполнения Заказчиком своих обязательств.`},
{t:"7. Обстоятельство непреодолимой силы",b:`7.1. Под обстоятельствами непреодолимой силы понимаются обстоятельства, которые могут возникнуть после заключения настоящего Договора в результате непредвиденных или непреодолимых Сторонами событий.
7.2. Стороны освобождаются от ответственности за частичное или полное неисполнение своих обязательств по настоящему Договору, если это неисполнение явилось следствием обстоятельств непреодолимой силы, а именно: чрезмерное воздействие атмосферных осадков и низких температур, повлекшие невозможность выполнения принятых на себя обязательств Сторонами; пожары; наводнения; землетрясения; войны; запретительные акты государственных органов, носящих общий характер, и их последствия, при условии, что эти обстоятельства и их последствия непосредственно повлияли на исполнение обязательств по настоящему Договору. В таком случае срок исполнения обязательств по настоящему Договору продлевается соразмерно времени, в течение которого действовали такие обстоятельства или их последствия при обязательном письменном уведомлении Сторонами друг друга.
7.3. Если последствия, вызванные указанными выше обстоятельствами, будут длиться более 3 (трех) месяцев с момента заключения настоящего Договора, то Стороны встретятся, чтобы обсудить, какие меры можно предпринять в создавшейся ситуации. Если в течение последующего одного месяца Стороны не смогут договориться по существу дела, то каждая из Сторон вправе расторгнуть настоящий Договор в одностороннем порядке.`},
{t:"8. Порядок расторжения Договора",b:`8.1. Настоящий Договор может быть расторгнут досрочно на основании письменного соглашения Сторон.
8.2. В случае расторжения настоящего Договора по инициативе Заказчика в период после начала Исполнителем выполнения работ по предмету настоящего Договора, последний документально подтверждает фактически выполненный объем таких работ и понесенные убытки, которые оплачиваются Заказчиком в полном объеме в течение 7 (семи) рабочих дней с момента надлежащего оформления Сторонами Акта взаиморасчетов, являющегося неотъемлемой частью настоящего Договора.
8.3. В случае расторжения настоящего Договора по инициативе Исполнителя в период после начала Исполнителем выполнения работ по предмету настоящего Договора, последний документально подтверждает фактически выполненный объем таких работ и фактические затраты.
8.4. В течение 7 (семи) рабочих дней с момента надлежащего оформления Сторонами Акта взаиморасчетов, являющегося неотъемлемой частью настоящего Договора, полученные от Заказчика денежные средства возвращаются Исполнителем за минусом цены фактически выполненного объема работ по предмету настоящего Договора и фактических понесенных Исполнителем затрат.`},
{t:"9. Порядок разрешения споров",b:`9.1. Все споры или разногласия, которые могут возникнуть в связи с исполнением или толкованием настоящего Договора, будут решаться путем переговоров между Сторонами. Стороны устанавливают обязательный претензионный (досудебный) порядок разрешения разногласий по настоящему Договору.
9.2. Претензия направляется в письменной форме в течение 10 (десяти) календарных дней с момента нарушения одной из Сторон обязательств по настоящему Договору с приложением документов (без приложения претензия не рассматривается), подтверждающих факт нарушения обязательств одной из Сторон. Претензия должна быть рассмотрена Стороной, которая получила претензию, в течение 5 (пяти) календарных дней с момента получения такой претензии.
9.3. В случае невозможности разрешения споров путем переговоров, они подлежат разрешению в судебном порядке по территориальной подсудности места нахождения Исполнителя.`},
{t:"10. Дополнительные условия",b:`10.1. В случае платного въезда на Строительную площадку Заказчика работников (представителей) Исполнителя для выполнения работ по предмету настоящего Договора, такие расходы оплачивает Заказчик.
10.2. Оплата электроэнергии, израсходованной Исполнителем в период выполнения работ по предмету настоящего Договора, осуществляется Исполнителем.
10.3. По согласованию Сторон (в случае необходимости), подготовка Строительной площадки работниками Исполнителя является «Дополнительными работами» и подлежит оплате Заказчиком на основании надлежаще оформленного Дополнительного соглашения, являющегося неотъемлемой частью настоящего Договора.
10.4. В случае, если Заказчик не готов принять объект на своем участке в течении 3 (трех) календарных дней с момента получения уведомления от Исполнителя о завершении всех работ по настоящему договору и письменно просит Исполнителя временно оставить объект на производственной площадке (либо уклоняется от приемки/вывоза), такой период считается хранением.
10.5. За период хранения (нахождения) объекта на производственной площадке Исполнителя сверх срока, установленного в п. 10.4, Заказчик обязуется уплатить Исполнителю плату за хранение в размере 50 000 (пятьдесят тысяч) рублей за каждый полный или неполный календарный месяц.
10.6. Стороны пришли к согласию о том, что древесина является специфическим строительным материалом, т.к. при обработке, изготовлении и монтаже нарушается ее структура, вследствие этого и чрезмерного воздействия влажной среды возможны: разбухание, появление трещин, появление синевы и деформации. Такие свойства древесины при выполнении работ по предмету настоящего Договора для Сторон не являются существенными.
10.7. С момента подписания настоящего Договора все ранее достигнутые устные договоренности, а также электронные и факсимильные сообщения теряют силу.`},
...
Работы по изготовлению {ПРЕДМЕТ} выполнены в полном объеме и в срок. Стороны никаких материальных и финансовых претензий друг к другу не имеют.
Настоящий Акт составлен в 2-х экземплярах, имеющих равную юридическую силу. Один экземпляр находится у Заказчика, второй экземпляр — у Исполнителя.`;
...
// Текущие тексты активного типа объекта: правка из settings или дефолт
function ctTplTexts(){
  const ov=(settings.ctTplTexts||{})[ctTplObjType().k];
  return {
...
// Изменяемая копия текстов в settings (создаётся из дефолта при первой правке) + автосейв
function ctTplTextsEnsure(){
  const k=ctTplObjType().k;
  const cur=(settings.ctTplTexts||{})[k];
  if(cur)return cur;
  const fresh={sections:CT_DOC_SECTIONS.map(function(s){return {t:s.t,b:s.b};}),act:CT_DOC_ACT};
  const all=Object.assign({},settings.ctTplTexts||{});
  all[k]=fresh;
  settings=Object.assign({},settings,{ctTplTexts:all});
  return fresh;
}
// Убрать правки: i — индекс раздела, "act" — акт, null — весь текст типа целиком
function ctTplTextsReset(i){
  const k=ctTplObjType().k;
  const all=Object.assign({},settings.ctTplTexts||{});
  if(!all[k])return;
...
// Plain text раздела → HTML печатной формы: esc всего текста, потом плейсхолдеры
// (значения уже безопасны), «- »-строки в <ul>, номер пункта — жирным.
function ctTplRenderBody(raw,vars){
  const lines=esc(String(raw||"")).split("\n");
  let html="",ul=[];
  function flushUl(){ if(ul.length){ html+="<ul>"+ul.map(function(x){return "<li>"+x+"</li>";}).join("")+"</ul>"; ul=[]; } }
  function subst(s){
    Object.keys(vars).forEach(function(key){ s=s.split("{"+key+"}").join(vars[key]); });
    return s;
  }
  lines.forEach(function(line){
    const s=line.trim();
    if(!s){ flushUl(); return; }
    if(s==="{ПЛАТЕЖИ}"){ flushUl(); html+=vars["ПЛАТЕЖИ"]||""; return; }
    if(s.indexOf("- ")===0){ ul.push(subst(s.slice(2))); return; }
    flushUl();
    html+="<p>"+subst(s).replace(/^(\d+(?:\.\d+)*\.)\s*/,"<b>$1</b> ")+"</p>";
  });
  flushUl();
  return html;
}
...
function ctTplGet(){
  if(ctTpl)return ctTpl;
  try{
    const raw=localStorage.getItem(CT_TPL_LS_KEY);
    if(raw)return ctTpl=Object.assign(ctTplDefaults(),JSON.parse(raw));
  }catch(e){}
  return ctTpl=ctTplDefaults();
}
function ctTplPersist(){
  try{ localStorage.setItem(CT_TPL_LS_KEY,JSON.stringify(ctTpl)); }catch(e){}
}

// Число прописью (до миллиардов). fem=true — женский род единиц («одна тысяча», «две»)
function numWordsRu(n,fem){
  n=Math.floor(Math.abs(Number(n)||0));
  const U1=["","один","два","три","четыре","пять","шесть","семь","восемь","девять"];
  const U1F=["","одна","две","три","четыре","пять","шесть","семь","восемь","девять"];
  const U10=["десять","одиннадцать","двенадцать","тринадцать","четырнадцать","пятнадцать","шестнадцать","семнадцать","восемнадцать","девятнадцать"];
  const T10=["","","двадцать","тридцать","сорок","пятьдесят","шестьдесят","семьдесят","восемьдесят","девяносто"];
  const H100=["","сто","двести","триста","четыреста","пятьсот","шестьсот","семьсот","восемьсот","девятьсот"];
  function triad(k,f){
    const w=[];
    if(Math.floor(k/100))w.push(H100[Math.floor(k/100)]);
    const r=k%100;
    if(r>=10&&r<20)w.push(U10[r-10]);
    else{ if(Math.floor(r/10))w.push(T10[Math.floor(r/10)]); if(r%10)w.push((f?U1F:U1)[r%10]); }
    return w.join(" ");
  }
  if(n===0)return "ноль";
  const parts=[];
  const bil=Math.floor(n/1e9)%1000, mil=Math.floor(n/1e6)%1000, th=Math.floor(n/1e3)%1000, un=n%1000;
  if(bil)parts.push(triad(bil,false)+" "+pluralRu(bil,"миллиард","миллиарда","миллиардов"));
  if(mil)parts.push(triad(mil,false)+" "+pluralRu(mil,"миллион","миллиона","миллионов"));
  if(th)parts.push(triad(th,true)+" "+pluralRu(th,"тысяча","тысячи","тысяч"));
  if(un)parts.push(triad(un,fem));
  return parts.join(" ").trim();
}
function pluralRu(k,f1,f2,f5){ const a=Math.abs(k)%100; if(a>10&&a<20)return f5; const b=a%10; if(b===1)return f1; if(b>=2&&b<=4)return f2; return f5; }
// Сумма прописью: 1900000 → «Один миллион девятьсот тысяч рублей 00 копеек»
function rubWords(n){
  n=Math.floor(Math.abs(Number(n)||0));
  let s=n===0?"ноль":numWordsRu(n,false);
  s=s.charAt(0).toUpperCase()+s.slice(1);
  return s+" "+pluralRu(n,"рубль","рубля","рублей")+" 00 копеек";
}
const RU_MONTHS_GEN=["января","февраля","марта","апреля","мая","июня","июля","августа","сентября","октября","ноября","декабря"];
// Результат дат идёт в HTML печатной формы: не-ISO значение (старый Safari рендерит
// type="date" как текстовое поле) экранируем, чтобы не пронести разметку мимо esc()
const ISO_DATE_RE=/^(\d{4})-(\d{2})-(\d{2})$/;
function dateRuWords(iso){ // 2026-04-21 → «21 апреля 2026 г.»
  if(!iso)return "«____» ____________ 20__ г.";
  const m=String(iso).match(ISO_DATE_RE);
  if(!m)return esc(String(iso));
  const mon=Math.min(Math.max(parseInt(m[2],10)||1,1),12);
  return parseInt(m[3],10)+" "+RU_MONTHS_GEN[mon-1]+" "+m[1]+" г.";
}
function dateRuShort(iso){ // 2026-04-21 → 21.04.2026
  if(!iso)return "";
  const m=String(iso).match(ISO_DATE_RE);
...
function ctTplPayCheckHtml(){
  const t=ctTplGet();
  const total=(t.payments||[]).reduce(function(a,p){return a+(Number(p.amount)||0);},0);
  const amount=Number(t.amount)||0;
  if(!amount&&!total)return '<span style="color:#9aabbf">Заполните сумму договора и платежи</span>';
  const ok=total===amount;
  return '<span style="color:'+(ok?"#27ae60":"#e74c3c")+';font-weight:700">Платежи: '+fmt(total)+' из '+fmt(amount)+(ok?" ✓":" — суммы не сходятся!")+'</span>';
}

// Форма подвкладки «Шаблон договора»
function tContractTemplate(){
  const t=ctTplGet();
  function inp(field,label,opts){
    ...
    return '<div style="flex:'+(opts.flex||"1")+';min-width:'+(opts.minw||"130px")+'">'+
      ...
      '<input id="ct-tpl-'+field+'" data-tpl="'+field+'" type="'+(opts.type||"text")+'"'+
  ...
  html+=card("💰 СУММА И СРОКИ",
    '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:6px">'+
      inp("amount","СУММА ДОГОВОРА ₽",{money:true})+
      inp("workDays","СРОК РАБОТ (РАБ. ДНЕЙ)",{type:"number"})+
      inp("warrantyMonths","ГАРАНТИЯ (МЕС.)",{type:"number"})+
    '</div>'+
    '<div id="ct-tpl-amount-words" style="font-size:11px;color:#7a9aaa;font-style:italic">'+esc(t.amount?rubWords(t.amount):"")+'</div>');

  // График платежей
  let payHtml='';
  (t.payments||[]).forEach(function(p,i){
    payHtml+='<div style="background:#fafbfc;border:1px solid #e6ecf3;border-radius:9px;padding:9px 11px;margin-bottom:7px">'+
      '<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px">'+
        '<span style="font-size:11px;font-weight:700;color:#2980b9;min-width:44px">3.2.'+(i+1)+'.</span>'+
        '<input data-tpl-pay="amount" data-i="'+i+'" type="text" inputmode="numeric" data-money="1" value="'+esc(fmtMoney(p.amount||""))+'" placeholder="Сумма ₽" style="flex:1;padding:6px 9px;border-radius:7px;border:1px solid #d0dae8;font-size:12px;outline:none;text-align:right">'+
        '<button data-a="ct-tpl-pay-del" data-i="'+i+'" style="width:26px;height:26px;background:transparent;border:1px solid #e74c3c44;border-radius:6px;cursor:pointer;color:#e74c3c;font-size:11px">✕</button>'+
      '</div>'+
      '<textarea data-tpl-pay="note" data-i="'+i+'" placeholder="Условия платежа (когда и на какой этап)" style="width:100%;padding:6px 9px;border-radius:7px;border:1px solid #e0e6ee;font-size:11px;height:52px;resize:vertical;outline:none;box-sizing:border-box;color:#4a5a6a">'+esc(p.note||"")+'</textarea>'+
    '</div>';
  });
  ...
  if(ctTplLastDeletedPay){
    payHtml+='<button data-a="ct-tpl-pay-restore" style="width:100%;padding:8px;background:#eafaf1;border:1px solid #27ae6055;border-radius:8px;cursor:pointer;font-size:12px;color:#1e8449;font-weight:700;margin-bottom:8px">↩ Восстановить удалённый платёж ('+fmtMoney(ctTplLastDeletedPay.pay.amount||0)+' ₽)</button>';
  }
  ...
  let txtHtml='<div style="display:flex;align-items:center;justify-content:space-between;gap:8px">'+
    '<span style="font-size:11px;color:'+(texts.isCustom?"#e67e22":"#7a9aaa")+';font-weight:600">'+
      (texts.isCustom?"✏️ Текст изменён (правки хранятся в облаке)":"Стандартный текст (из договора-образца)")+
    '</span>'+
    '<button data-a="ct-tpl-text-toggle" style="padding:6px 14px;background:'+(ctTplTextEdit?"#7a9aaa":"#2980b9")+';border:none;border-radius:8px;cursor:pointer;font-size:12px;color:#fff;font-weight:700;white-space:nowrap">'+(ctTplTextEdit?"Свернуть":"✏️ Редактировать текст")+'</button>'+
  '</div>';
  if(ctTplTextEdit){
    txtHtml+='<div style="background:#fdf6ec;border:1px solid #e67e2244;border-radius:8px;padding:8px 10px;margin-top:10px;font-size:10.5px;color:#8a6a3a;line-height:1.5">'+
      'Правки сохраняются автоматически — отдельно для 🛁 Бани и 🏠 Дома. Подстановки из формы: '+
      '<b>{ПРЕДМЕТ}</b> <b>{СУММА}</b> <b>{СУММА_ПРОПИСЬЮ}</b> <b>{ПЛАТЕЖИ}</b> <b>{СРОК}</b> <b>{СРОК_ПРОПИСЬЮ}</b> <b>{ГАРАНТИЯ}</b> <b>{ГАРАНТИЯ_ПРОПИСЬЮ}</b> <b>{ГАРАНТИЯ_ЕД}</b> <b>{ИСПОЛНИТЕЛЬ}</b> <b>{ЗАКАЗЧИК}</b>. '+
      'Строка «- …» превращается в пункт списка, номер пункта (5.1.) выделяется жирным сам.'+
    '</div>';
    texts.sections.forEach(function(s,i){
      txtHtml+='<div style="background:#fafbfc;border:1px solid #e6ecf3;border-radius:9px;padding:9px 11px;margin-top:8px">'+
        '<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px">'+
          '<input data-tpltext-t="1" data-i="'+i+'" value="'+esc(s.t)+'" style="flex:1;padding:6px 9px;border-radius:7px;border:1px solid #d0dae8;font-size:12px;font-weight:700;outline:none;color:#1a2a3a">'+
          '<button data-a="ct-tpl-text-reset" data-i="'+i+'" title="Вернуть стандартный текст раздела" style="width:28px;height:28px;background:transparent;border:1px solid #d0dae8;border-radius:6px;cursor:pointer;color:#7a9aaa;font-size:13px">↺</button>'+
        '</div>'+
        '<textarea data-tpltext-b="1" data-i="'+i+'" style="width:100%;padding:8px 10px;border-radius:7px;border:1px solid #e0e6ee;font-size:11.5px;line-height:1.5;height:150px;resize:vertical;outline:none;box-sizing:border-box;color:#3a4a5a">'+esc(s.b)+'</textarea>'+
      '</div>';
    });
    txtHtml+='<div style="background:#fafbfc;border:1px solid #e6ecf3;border-radius:9px;padding:9px 11px;margin-top:8px">'+
      '<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px">'+
      ...
      '</div>'+
      '<textarea data-tpltext-act="1" style="width:100%;padding:8px 10px;border-radius:7px;border:1px solid #e0e6ee;font-size:11.5px;line-height:1.5;height:110px;resize:vertical;outline:none;box-sizing:border-box;color:#3a4a5a">'+esc(texts.act)+'</textarea>'+
    '</div>';
...
// Синк полей формы шаблона в буфер ctTpl (поля живут в innerHTML и гибнут при render —
// тот же паттерн, что _bindContractFormSync). Money-поля дополнительно форматирует
// глобальный _bindMoneyInputs; здесь читаем уже отформатированное значение через unfmtMoney.
function _bindCtTplSync(){
  const t=ctTplGet();
  document.querySelectorAll("[data-tpl]").forEach(function(el){
    ...
    const sync=function(){
      let v=el.value;
      if(el.dataset.money==="1")v=unfmtMoney(v);
      else if(el.type==="number")v=parseInt(v,10)||0;
      t[el.dataset.tpl]=v;
      ctTplPersist();
      if(el.dataset.tpl==="amount"){
        const w=document.getElementById("ct-tpl-amount-words");
        if(w)w.textContent=v?rubWords(v):"";
        const chk=document.getElementById("ct-tpl-pay-check");
        if(chk)chk.innerHTML=ctTplPayCheckHtml();
      }
    };
    el.addEventListener("input",sync);
    el.addEventListener("change",sync);
  });
  document.querySelectorAll("[data-tpl-pay]").forEach(function(el){
    if(el._tplSync)return;
    el._tplSync=true;
    const sync=function(){
      const i=parseInt(el.dataset.i,10);
      const p=t.payments&&t.payments[i];
      if(!p)return;
      if(el.dataset.tplPay==="amount"){
        p.amount=unfmtMoney(el.value);
        const chk=document.getElementById("ct-tpl-pay-check");
        if(chk)chk.innerHTML=ctTplPayCheckHtml();
      } else {
        p.note=el.value;
  ...
  // Редактор текста договора: правки → settings.ctTplTexts (синк в облако, debounce apiSave)
  function bindTplText(sel,apply){
    document.querySelectorAll(sel).forEach(function(el){
      if(el._tplSync)return;
      el._tplSync=true;
      const sync=function(){ apply(el); scheduleSave(); };
      el.addEventListener("input",sync);
      el.addEventListener("change",sync);
    });
  }
  bindTplText("[data-tpltext-b]",function(el){
    const s=ctTplTextsEnsure().sections[parseInt(el.dataset.i,10)];
    if(s)s.b=el.value;
  });
  bindTplText("[data-tpltext-t]",function(el){
    const s=ctTplTextsEnsure().sections[parseInt(el.dataset.i,10)];
    if(s)s.t=el.value;
  });
...
// переменные части (реквизиты заказчика, суммы, сроки, график) подставляются из формы.
...
function ctTplDocHtml(){
  ...
  function payItems(prefix){
    return pays.map(function(p,i){
      const kind=(i<pays.length-1)?"авансовый платеж":"платеж";
      return "<p><b>"+prefix+(i+1)+".</b> "+kind+" в размере <b>"+fmtMoney(p.amount||0)+" ("+rubWords(p.amount||0)+")</b> — "+esc(p.note||"")+"</p>";
    }).join("");
  }
  ...
  const appHead=function(n){return '<p class="app-head">Приложение № '+n+' к договору № '+num+'</p>';};
  // Тексты разделов (правки из settings или дефолт) + значения плейсхолдеров
  const texts=ctTplTexts();
  const vars={
    "ПРЕДМЕТ":subject,
    "СУММА":fmtMoney(amount),
    "СУММА_ПРОПИСЬЮ":rubWords(amount),
    "ПЛАТЕЖИ":payItems("3.2."),
    "СРОК":String(wd),
    "СРОК_ПРОПИСЬЮ":numWordsRu(wd,false),
    "ГАРАНТИЯ":String(wm),
    "ГАРАНТИЯ_ПРОПИСЬЮ":numWordsRu(wm,false),
    "ГАРАНТИЯ_ЕД":pluralRu(wm,"месяц","месяца","месяцев"),
    "НОМЕР":num,
    "ИСПОЛНИТЕЛЬ":ex.preamble,
    "ЗАКАЗЧИК":client
  };
  const sectionsHtml=texts.sections.map(function(s){
    return '<div class="sec">'+esc(s.t)+'</div>'+ctTplRenderBody(s.b,vars);
  }).join("");

  return '<!DOCTYPE html><html lang="ru"><head><meta charset="utf-8"><title>Договор подряда № '+num+'</title><style>'+
    '@page{size:A4;margin:16mm 15mm 22mm}'+
    'body{font-family:"Times New Roman",Times,serif;font-size:12.5px;line-height:1.5;color:#000;margin:0;padding:24px 28px 60px}'+
    'h1{font-size:15px;text-align:center;margin:0 0 2px}'+
    '.dateline{display:flex;justify-content:space-between;margin:10px 0 14px;font-weight:bold}'+
    '.sec{font-weight:bold;text-align:center;margin:16px 0 6px;font-size:13px}'+
    'p{margin:5px 0;text-align:justify}'+
    'ul{margin:5px 0 5px 22px;padding:0}'+
    'li{margin:2px 0;text-align:justify}'+
    '.sig{display:flex;justify-content:space-between;margin:26px 0 10px;font-weight:bold}'+
    '.req{width:100%;border-collapse:collapse;margin-top:8px}'+
    '.req td{width:50%;vertical-align:top;padding:8px 10px;border:1px solid #000;font-size:12px}'+
    '.req .rh{font-weight:bold;text-align:center}'+
    '.app{page-break-before:always}'+
    '.app-head{text-align:right;font-weight:bold;margin:0 0 12px}'+
    ...
    '<div class="sec">График платежей</div>'+
    '<p><b>1.</b> По согласованию Сторон цена работ по предмету настоящего Договора составляет: <b>'+fmtMoney(amount)+' ('+rubWords(amount)+')</b>.</p>'+
    '<p><b>2.</b> По согласованию Сторон расчет по настоящему Договору производится в соответствии с Графиком платежей (Приложение № 2 к настоящему Договору) в валюте РФ (рубли) способом, не противоречащим действующему законодательству РФ, следующим образом:</p>'+
    pays.map(function(p,i){
      const kind=(i<pays.length-1)?"Авансовый платеж":"Платеж";
      return "<p><b>"+(i+3)+".</b> "+kind+" в размере <b>"+fmtMoney(p.amount||0)+" ("+rubWords(p.amount||0)+")</b> — "+esc(p.note||"")+"</p>";
    }).join("")+
    sig+'</div>'+
...
function bind(){
  ...
  document.querySelectorAll("[data-a]").forEach(el=>{
    ...
    // ─── Шаблон договора (подвкладка) ───
      ...
      const i=parseInt(el.dataset.i,10);
      const removed=ctTplGet().payments.splice(i,1)[0];
      if(removed)ctTplLastDeletedPay={pay:removed,index:i};   // запоминаем для «Восстановить»
      ctTplPersist();render();
    };}
    else if(a==="ct-tpl-pay-restore"){el.onclick=()=>{
      if(!ctTplLastDeletedPay)return;
      const pays=ctTplGet().payments;
      const idx=Math.min(Math.max(ctTplLastDeletedPay.index,0),pays.length);
      pays.splice(idx,0,ctTplLastDeletedPay.pay);   // возвращаем на прежнее место
      ctTplLastDeletedPay=null;
      ctTplPersist();render();
    };}
    ...
    else if(a==="ct-tpl-text-reset-all"){el.onclick=()=>{
      if(!confirm("Убрать ВСЕ правки текста для «"+ctTplObjType().n+"» и вернуть стандартный договор-образец?"))return;
      ctTplTextsReset(null);
      render();
    };}
    else if(a==="ct-tpl-reset"){el.onclick=()=>{if(confirm("Сбросить все поля шаблона договора?")){ctTpl=ctTplDefaults();ctTplPersist();render();}};}
    else if(a==="ct-tpl-crm"){el.onchange=()=>{
...
// Bootstrap. Сессия определяется ПЕРСОНАЛЬНЫМ токеном (сотрудник uid / клиент cid) в localStorage.
// Нет валидного токена → экран входа (сотрудник его получает по телефону+PIN, клиент — по договору+PIN).
function _autoLogin(){
...
(async function boot(){
  ...
  // (1) Клиентская сессия — кабинет клиента по клиентскому токену (сервер отдаёт только его срез).
  if (decoded && decoded.typ === "client"){
    _hydrated = true;
    try{
      const items = await apiLoad();
      if (items){ applyState(items); clientAuthContract = decoded.cid; }
      else { clearToken(); }
    }catch(e){ /* офлайн — покажем вход */ clearToken(); }
    render();
    return;   // клиент read-only: без автосейва/поллинга
  }
  ...
  // (3) Токен сотрудника есть, кэша нет (первый вход на устройстве / после отсечки) — грузим и стартуем.
  ...
  // (4) Нет валидного персонального токена (в т.ч. старый общий токен после отсечки) → экран входа.
  clearToken();
  _hydrated = true;

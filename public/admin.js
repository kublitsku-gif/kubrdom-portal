...
function applyState(items){
  ...
  // Claude отключён (нет ключа) — если он был выбран, откатываем на DeepSeek, чтобы нейропродавец не падал.
  if (settings && settings.aiProvider === "claude") settings = Object.assign({}, settings, { aiProvider: "deepseek" });
}
...
function tMarketingMain(){
  ...
  html+='<div style="background:linear-gradient(135deg,#005bff15,#00aaff08);border:1.5px solid #005bff33;border-radius:14px;padding:16px;margin-bottom:14px">'+
    ...
    '<div style="display:flex;align-items:center;gap:6px;margin-bottom:12px;flex-wrap:wrap">'+
      ...
      '</div>'+
    '</div>'+
    (function(){var p=(["kimi","gpt"].indexOf(settings.aiProvider)>=0)?settings.aiProvider:"deepseek";function pb(k,lbl,col){var on=p===k;return '<button data-a="mkt-provider" data-p="'+k+'" style="flex:1;min-width:70px;padding:7px 4px;border-radius:8px;cursor:pointer;font-size:11px;font-weight:700;border:1.5px solid '+(on?col:"#dde6f0")+';background:'+(on?col:"#fff")+';color:'+(on?"#fff":"#7a9aaa")+'">'+lbl+'</button>';}var claudeBtn='<button disabled title="Нужен ключ Anthropic" style="flex:1;min-width:70px;padding:7px 4px;border-radius:8px;cursor:not-allowed;font-size:11px;font-weight:700;border:1.5px solid #e6e6e6;background:#f3f3f3;color:#bcbcbc">Claude 🔒</button>';return '<div style="padding:9px 11px;background:#fff;border:1px solid #dde6f0;border-radius:9px;margin-bottom:8px"><div style="font-size:12px;font-weight:700;color:#1a2a3a;margin-bottom:6px">Модель ИИ</div><div style="display:flex;flex-wrap:wrap;gap:6px">'+pb("deepseek","DeepSeek","#16a085")+pb("kimi","Kimi","#6c5ce7")+pb("gpt","GPT","#0ea5e9")+claudeBtn+'</div></div>';})()+

...
// ── 3. Виды и липкие шапки ──────────────────────────────────────────────────
{
  ...
  // Материалы одной работы — подряд и в порядке сметы, а не по алфавиту: по алфавиту
  // «Брус» вставал перед «Разъёмом», и электрика разъезжалась по списку.
  const iR = stage.indexOf('Разъём 32А'), iK = stage.indexOf('Кабель ВВГ'), iB = stage.indexOf('Брус 50×100')
  t.ok('строки этапа идут по работам в порядке сметы', iR >= 0 && iR < iK && iK < iB, [iR, iK, iB].join(' < '))
  const label = (w, n) => new RegExp('↳ ' + w + '</span><span[^>]*>· ' + n + ' поз\\.')
  t.ok('над блоком работы подпись с числом позиций', label('Разводка электрики', 2).test(stage) && label('Каркас', 1).test(stage))
  t.ok('подпись стоит перед своими строками', stage.search(label('Разводка электрики', 2)) < iR && stage.search(label('Каркас', 1)) < iB
    && stage.search(label('Каркас', 1)) > iK)
  t.ok('в виде «Работы» подпись не дублирует шапку', !label('Разводка электрики', '\\d+').test(work))
  const merge = view(p, 'merge')
  t.ok('в «Закупке» внутри магазина тоже по работам', merge.indexOf('Разъём 32А') < merge.indexOf('Кабель ВВГ'))
}

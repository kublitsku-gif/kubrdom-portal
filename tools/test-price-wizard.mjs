...
// ── Карточка мастера собрана под ввод ───────────────────────────────────────
// Порядок на карточке — это порядок действий: открыть магазин, сказать, чем он
// торгует, вписать цену. Ввод цены переводит на следующую карточку, поэтому
// делитель обязан стоять ДО неё.
{
  t.section('Порядок и ссылки в карточке мастера')
  const p = panel()
  const stN = stageOf(p)
  const open = p.dom.node({ a: 'price-wiz-open', n: String(stN) }); p.run('bind();'); open.onclick()
  const html = p.run('tSpec2()')
  ...
  t.ok('упаковка спрашивается до цены',
    ...
    'поле цены стоит раньше делителя')

  // Магазин, открытый большой кнопкой, в ряду «сравнить» не повторяется: две

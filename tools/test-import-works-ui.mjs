...
// «Баня»: форма «+ работа» их не видит, берём через ту же панель.
{
  ...
  t.ok('из базы ничего не отмечено заранее — там варианты', /Отметьте работы/.test(html), (html.match(/Взять[^<]*/) || [''])[0])
  click(p, { a: 'est-import-pick', k: 'add:kb_e_bath' })
  t.ok('отметили руками', /Взять 1 работу/.test(p.run('tProjects()')))
  click(p, { a: 'est-import-do' })

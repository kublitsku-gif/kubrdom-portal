...
// ── 1. Цепочки сходятся ──────────────────────────────────────────────────────
{
  ...
  // Цепочка проёмов меряет РЕЗ в стене: окно уже реза на два монтажных зазора И ДВЕ
  // ТРУБЫ — трубы стоят В САМОМ РЕЗЕ и потому в него входят (узел 1).
  ...
  ok('цепочка проёмов длинной стены — по резу и с привязками',
    bot.segs.join(' ') === '2176 990 1120 930 1120 2240 3376', bot.segs.join(' '))
  ...
  ok('рез шире изделия на два зазора И ДВЕ ТРУБЫ',
    sc.openings.filter((o) => o.frame).every((o) => o.cutW === o.width + 120),
    sc.openings.filter((o) => o.frame).map((o) => o.width + '->' + o.cutW).join(', '))
...
// зазора — по этим числам режут стену, и ошибка здесь стоит переваренного проёма.
{
  ...
  // Труба стоит В РЕЗЕ и делится наружной гранью листа ПОПОЛАМ: по 30 мм наружу и
  // внутрь. На плане она обязана оседлать линию наружной стены — нарисованная целиком
  // снаружи или целиком в толще, она сказала бы бригаде не ту глубину посадки.
  ok('и оседлала линию наружной стены — по 30 мм в каждую сторону', win1.jambs.every((j) => {
    const face = (win1.side === 'n') ? win1.y : win1.y + win1.h;
    return j.y === face - 30 && j.y + j.h === face + 30;
  ...
  // В торце посадка ложится вдоль длины дома, а не поперёк: сторона у трубы одна и та
  // же, меняется только то, куда смотрит наружная грань стены.
  ok('и в торце труба тоже оседлала стену',
    endOn.jambs.every((j) => j.w === 60 && j.h === 40 &&
      j.x === endOn.x + endOn.w - 30 && j.x + j.w === endOn.x + endOn.w + 30),
    JSON.stringify(endOn.jambs))
}
...
// ── 4. Ничего лишнего ────────────────────────────────────────────────────────
{
  ...
  // это сторожит `test-scheme-tab` на самом SVG.

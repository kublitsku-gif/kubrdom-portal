...
// серия — свойство этой стройки, а не портала. Один дом — колонок серии нет вовсе.
{
  ...
  // Счёт читает ЗАКАЗЧИК. «Компенсация» — наш внутренний термин: он про деньги,
  // которые к нам вернутся, а заказчика просят заплатить. В документе его быть
  // не должно ни в одном месте — заголовке, пояснении, итогах.
  t.ok('слова «компенсация» в счёте нет', !/компенс/i.test(many),
    'в документе для заказчика остался внутренний термин')
  t.ok('документ назван оплатой', /Счёт на оплату/.test(many),
    'счёт должен называть себя оплатой')

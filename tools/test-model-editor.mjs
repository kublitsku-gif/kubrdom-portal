...
// ── 22. Толщина стены — один раз на чертёж ───────────────────────────────────
// Пирог у всех стен один: обшивка 76, перегородка 100. Повторённое у каждого угла
// число ничего не добавляет, зато забивает цепочки так, что перестают читать и
// нужные размеры. Толщина при этом никуда не девается — она в цепочке перегородок
// и в узлах, где ей и место.
{
  t.section('Толщина стены один раз')
  const p = panel()
  const svg = p.run('modelSchemeSvg(specSheets[0].model, winTypes)')
  const m = p.q('(function(){var s=specSheets[0].model;return [s.finish,s.wallThick];})()')
  const cnt = (v) => (svg.match(new RegExp('>' + v + '<', 'g')) || []).length
  t.ok('обшивка названа ровно раз', cnt(m[0]) === 1, m[0] + ' × ' + cnt(m[0]))
  // Перегородка — тоже раз в наружных цепочках. Внутренняя цепочка двери может
  // честно показать такое же число: там это не толщина, а отступ от стены до

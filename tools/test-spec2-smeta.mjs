...
// Шапка сметы: себестоимость и её раскладка. Разбираем готовый HTML — сторожим
// ПОКАЗАННОЕ: раскладка стоит под себестоимостью (а не под ценой клиенту, которую
// она не делит), и её числа сходятся с итогом.
...
// ── 6. Вкладка в панели ─────────────────────────────────────────────────────
{
  ...
  // что показали одну цену, а собрали по другой.
  const head = costHead(est)
  t.ok('себестоимость разложена на материалы и работу', !!head, String(head))
  t.ok('и вместе они дают её же',
  ...
  t.ok('площади по помещениям тоже', /ПО ПОМЕЩЕНИЯМ/.test(estOpen))
  t.ok('комнаты названы', /Санузел/.test(estOpen) && /Спальня/.test(estOpen))
  t.ok('стены двумя числами', /СТЕНЫ ЧИСТЫЕ/.test(estOpen) && /СТЕНЫ ПОЛНЫЕ/.test(estOpen))
  t.ok('и потолок отдельной колонкой', /ПОТОЛОК/.test(estOpen))
  ...
  const noOps = p.run('factsOpen=true;spec2FactsHtml({height:2.5,'
    + 'rooms:[{id:"r1",name:"Зал",floor:10,ceil:10,wallNet:20,wallGross:20,openings:0}],'
    + 'total:{floor:10,ceil:10,wallNet:20,wallGross:20,openings:0},points:[],openings:[]})')
  t.ok('без проёмов стены одной колонкой', /СТЕНЫ<\/div>/.test(noOps) && !/СТЕНЫ ПОЛНЫЕ/.test(noOps))
  t.ok('и сказано почему', /Проёмов в доме нет/.test(noOps))
...
// пришлось бы повторять на каждом доме, поэтому переключатель.
{
  ...
  t.ok('все три остались отдельными строками',
    (before.match(/ППУ \d см/g) || []).length >= 3)
  ...
  // На пилюле — на сколько поедет дом, если переключить; сама цена — в подсказке.
  t.ok('у каждого своя цена', /ППУ 8 см · [+−][\d\s  ]+ ₽/.test(html) &&
    /title="Утепление стен и потолка: ППУ 8 см — 4[\s  ]500 ₽"/.test(html))

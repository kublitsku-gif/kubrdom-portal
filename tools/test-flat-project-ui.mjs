...
// ── 3. Полоса «Чертёж» у квартиры ────────────────────────────────────────────
{
  ...
  // Чертить квартиру портал не умеет и не должен: кнопка вела бы в редактор
  // контейнера, где её планировке не на что лечь.
  t.ok('в редактор чертежа не зовут', plan.indexOf('data-a="proj-edit"') < 0)
  t.ok('полоса подписана по делу', /что отделать/.test(plan))
  t.ok('площади — по квартире, а не по дому', /ВСЕГО ПО КВАРТИРЕ/.test(plan))
...
// ── 4. Замер правится руками ─────────────────────────────────────────────────
{
  ...
  // Г-образная комната: нулей вместо размеров быть не должно — это не размер,
  // а его отсутствие, и «0×0 м» читается как ошибка ввода.
  addRoom(p, pid, { name: 'Спальня', floor: '11,79', wallLen: '15,55' })
  const withL = p.run('tProjects()')
  t.ok('у непрямоугольной комнаты нулей нет', withL.indexOf('0×0 м') < 0)
  t.ok('и сказано, что она не прямоугольная', /не прямоуг\./.test(withL))

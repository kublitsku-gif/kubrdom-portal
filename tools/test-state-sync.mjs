...
// ── 9. Отметка сохранения говорит про облако, а не про нажатие ──────────────
// Зелёное «Сохранено» загоралось на КАЖДЫЙ тап: его зажигал дебаунс, то есть любая
// перерисовка. Отметка, которая горит всегда, перестаёт отвечать на единственный
// вопрос, который к ней есть, — мои правки в облаке или нет.
{
  t.section('Отметка сохранения')
  const net = server()
  const p = panel(net)
  t.ok('поле статуса живёт в шапке, а не всплывашкой в углу',
    p.run('typeof saveMarkHtml==="function" && saveMarkHtml().indexOf(\'id="save-mark"\')>=0'))
  t.ok('всплывающей плашки больше нет', p.run('render(); document.getElementById("save-toast")') === null)

  // Дебаунс сам по себе ничего не обещает: он срабатывает и там, где нечего слать.
  p.run('saveMark="idle";scheduleSave();')
  t.ok('дебаунс отметку не красит', p.q('saveMark') === 'idle', 'получили: ' + p.q('saveMark'))

  // Пустой прогон — нечего сохранять, значит и говорить не о чем.
  const r0 = await p.ctx.apiSave()
  t.ok('пустой сейв статус не трогает', r0.skipped === true && p.q('saveMark') === 'idle',
    'получили: ' + p.q('saveMark'))

  // Настоящая правка — «сохранено» появляется ПОСЛЕ ответа сервера.
  p.run('objects[0].name="Правка ради отметки";')
  await p.ctx.apiSave()
  t.ok('после подтверждения — сохранено', p.q('saveMark') === 'ok', 'получили: ' + p.q('saveMark'))
  t.ok('и со временем подтверждения', p.q('saveMarkAt') > 0)
  t.ok('в лицо статуса попало время', /\d\d:\d\d/.test(p.run('saveMarkFace().text')),
    'получили: ' + p.run('saveMarkFace().text'))
  t.ok('и первые секунды оно зелёное', p.run('saveMarkFace().color') === '#27ae60')
  // Зелёный — про «прямо сейчас»: на экране, открытом с утра, он врал бы каждым взглядом.
  p.run('saveMarkAt=Date.now()-60000;')
  t.ok('через минуту статус спокойный', p.run('saveMarkFace().color') === '#9aabbf',
    'получили: ' + p.run('saveMarkFace().color'))
  t.ok('но время сохранения на месте', /\d\d:\d\d/.test(p.run('saveMarkFace().text')))
}

// ── 10. Отказ сервера виден в статусе ───────────────────────────────────────
{
  t.section('Не сохранено — это состояние')
  const net = server({ reply: () => ({ status: 500, json: { error: 'boom' } }) })
  const p = panel(net)
  p.run('objects[0].name="Правка в мёртвую связь";')
  await p.ctx.apiSave()
  t.ok('статус говорит «не сохранено»', p.q('saveMark') === 'wait', 'получили: ' + p.q('saveMark'))
  t.ok('и не выдаёт это за успех', p.run('saveMarkFace().color') !== '#27ae60')
}

// ── 11. Экранный тумблер — не правка ────────────────────────────────────────
// Свёрнутый блок и раскрытая справка живут на экране и в снимок не идут. Каждый
// такой тап сериализовал весь снимок, гонял стражей и перерисовывал страницу.
{
  t.section('Экран отдельно, сохранение отдельно')
  const net = server()
  const p = panel(net)
  t.ok('у экранной перерисовки своя дверь', p.run('typeof ui==="function"'))
  p.run('render();')
  // Считаем обращения к автосейву: тумблер экрана не должен добираться до него вовсе.
  p.run('window.__saves=0; scheduleSave=function(){ window.__saves++; };')
  p.run('factsOpen=!factsOpen;ui();')
  t.ok('ui() сохранение не заводит', p.q('window.__saves') === 0,
    'иначе тумблер экрана снова стучится в облако')
  t.ok('и статус не трогает', p.q('saveMark') === 'idle', 'получили: ' + p.q('saveMark'))
  // А правка данных по-прежнему сохраняется.
  p.run('objects[0].name="Настоящая правка";fl();')
  t.ok('fl() автосейв заводит', p.q('window.__saves') === 1, 'получили: ' + p.q('window.__saves'))
}

...
// по-разному: вторая копия чертежа разошлась бы с первой на первой же правке.
{
  ...
  // Главное требование узла: труба НЕ касается железа контейнера. Приваренная к
  // листу труба — сквозной металл с улицы внутрь, и заливка его уже не лечит.
  t.ok('сказано, что железа труба не касается', /не касается листа/.test(dim))

...
// по-разному: вторая копия чертежа разошлась бы с первой на первой же правке.
{
  ...
  // Два размера, и путать их нельзя: стену режут ЗА трубой, а окно заказывают по
  // проёму между трубами. Раньше на узле стоял только рез, подписанный «проём».
  t.ok('на узле стоит рез в стене', /рез в стене = окно \+ 110…120/.test(dim), 'по нему режут железо')
  t.ok('и проём между трубами', /проём между трубами = окно \+ 30…40/.test(dim), 'по нему заказывают окно')
  ...
  t.ok('и в нём стоят числа', /110…120/.test(dim) && /15…20/.test(dim) && /рез шире окна на 110–120 мм/.test(dim))
  ...
  t.ok('в узле проёма стена идёт гофрой', /лист контейнера/.test(jamb) && zig(jamb) > 20, String(zig(jamb)))

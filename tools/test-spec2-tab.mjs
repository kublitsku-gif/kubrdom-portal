...
// по-разному: вторая копия чертежа разошлась бы с первой на первой же правке.
{
  ...
  t.ok('и что швов два', /сварной шов с обеих сторон листа/.test(dim) && /снаружи и изнутри/.test(dim))
  // Труба пересекает лист, и на узле обязана быть сказана цена этого решения: по
  // контуру проёма идёт сквозной металл. Молчать об этом — врать бригаде, которая
  // по прошлому чертежу читала обратное.
  t.ok('и чем за это платят', /ППУ/.test(dim) && /мостик холода/.test(dim) && /сквозной металл/.test(dim))

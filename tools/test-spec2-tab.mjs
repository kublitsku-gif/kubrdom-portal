...
// по-разному: вторая копия чертежа разошлась бы с первой на первой же правке.
{
  ...
  // Труба стоит В САМОМ РЕЗЕ, значит рез ШИРЕ проёма между трубами на две трубы, и
  // на узле это ДВА разных размера. Один размер на оба — как было, пока труба лежала
  // накладкой снаружи, — заставил бы вырезать на 80 мм меньше, и трубу в такой рез
  // уже не вставить.
  t.ok('на узле два размера: рез и проём между трубами',
    /рез в стене = окно \+ 110…120/.test(dim) && /проём между трубами = окно \+ 30…40/.test(dim))
  t.ok('и они не путаются', dim.indexOf('рез в стене = проём между трубами') < 0)
  t.ok('цепочка подписана окном, а не «изделием»', />окно</.test(dim) && dim.indexOf('>изделие<') < 0)
  // Шов до окна — не пустота и НЕ один материал: снаружи полиуретановый герметик,
  // изнутри монтажная пена. Пену, выпущенную на улицу, за сезон съедает ультрафиолет,
  // поэтому на узле это два разных куска, а не подпись «заполнить пеной».
  t.ok('шов показан двумя материалами', /jamb-foam/.test(dim) && /jamb-seal/.test(dim))
  t.ok('и подписан, что где', /полиуретановый герметик — снаружи/.test(dim) && /монтажная пена — изнутри/.test(dim))
  t.ok('и в тексте под узлом это написано',
    /снаружи полиуретановый герметик/.test(dim) && /изнутри монтажная пена/.test(dim))
  t.ok('и в нём стоят числа', /15…20/.test(dim) && /рез ШИРЕ проёма между трубами на две трубы/.test(dim))
  // Труба стоит СНАРУЖИ листа и варится к нему с обеих сторон — это и есть узел.
  // Внутрь стены её не заводят: труба в толще ППУ была бы сквозным металлом с улицы
  // в комнату. Без этой причины на площадке приварят как удобнее.
  t.ok('сказано, что труба в резе по центру листа', /в самом резе, по центру листа/.test(dim))
  t.ok('и что шов только снаружи', /сварной шов только снаружи/.test(dim) && dim.indexOf('с обеих сторон листа') < 0)
  // Труба пересекает лист — и мостик холода снимает не молчание, а ППУ: он заходит
  // в проём на ширину трубы и обходит её изнутри, замыкая контур. Узел обязан
  // показывать И заход, И цену — над трубой слоя вдвое меньше, чем по стене.
  t.ok('и чем закрыт металл',
    /ППУ/.test(dim) && /обходит трубу/.test(dim) && /мостика холода нет/.test(dim))
  t.ok('и где контур тоньше всего', /над трубой остаётся 20 мм/.test(dim))
  t.ok('и что труба входит в рез', /по 30 мм наружу и внутрь/.test(dim))
  ...
  // Утеплитель на узле — слой, а не фон: по нему считают материал, и труба сидит
  // именно в нём. Раньше за листом была просто штриховка «стена».
  t.ok('утеплитель показан за листом', /jamb-ppu/.test(jamb) && />ППУ</.test(jamb))
  ...
  t.ok('таблица проёмов с колонкой реза', /Рез в стене/.test(sheet) && /1120 × 2100/.test(sheet))
  ...
  t.ok('и подписаны размером', />1000×2100</.test(psvg) && />2160×2390</.test(psvg))
  // Дверь размером не подписана — её читают по створке. Проверяем счётом: окно и

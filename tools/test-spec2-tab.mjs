...
// по-разному: вторая копия чертежа разошлась бы с первой на первой же правке.
{
  ...
  t.ok('усиление на рабочем чертеже', (dsvg.match(/fill="#2f4a63"/g) || []).length === 4,
    String((dsvg.match(/fill="#2f4a63"/g) || []).length))
  t.ok('и его нет на плане клиента', psvg.indexOf('fill="#2f4a63"') < 0)
  t.ok('числа узла подписаны рядом с чертежом', /Усиление проёмов/.test(dim) && /110–120 мм шире изделия/.test(dim))
  t.ok('клиенту про трубы не рассказываем', pl.indexOf('Усиление проёмов') < 0)
}

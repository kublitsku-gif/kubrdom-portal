...
// по-разному: вторая копия чертежа разошлась бы с первой на первой же правке.
{
  ...
  const tubes = (h) => (h.match(/<rect[^>]*fill="#2f4a63"/g) || []).length
  t.ok('усиление на рабочем чертеже', tubes(dsvg) === 4, String(tubes(dsvg)))
  t.ok('и его нет на плане клиента', psvg.indexOf('#2f4a63') < 0)
  ...
  t.ok('и в нём стоят числа', /110…120/.test(dim) && /15…20/.test(dim) && /110–120 мм шире изделия/.test(dim))

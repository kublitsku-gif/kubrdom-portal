...
// по-разному: вторая копия чертежа разошлась бы с первой на первой же правке.
{
  ...
  t.ok('сказано, что труба снаружи листа', /снаружи листа/.test(dim))
  t.ok('и что швов два', /сварной шов/.test(dim) && /по обеим сторонам трубы/.test(dim))
  t.ok('и почему именно так', /ППУ/.test(dim) && /мостик[а]? холода/.test(dim))
  t.ok('и что вылет в ширину реза не входит', /вылет наружу/.test(dim))
  ...
  t.ok('на чертеже подписан рез', /рез 1040/.test(dim2))
  t.ok('и цепочка меряет его же', /<text[^>]*>1040</.test(dim2))
  ...
  t.ok('таблица проёмов с колонкой реза', /Рез в стене/.test(sheet) && /1040 × 2100/.test(sheet))

...
// ── Петрович узнаётся по ссылке ─────────────────────────────────────────────
// Ссылку копируют с той версии сайта, что открыта у человека, а у Петровича
// адрес региональный. Не узнай мы поддомен — в карточку уехало бы предложение
// магазина «moscow.petrovich.ru», и вторая такая карточка уже не склеилась бы
// с первой.
{
  t.section('Петрович узнаётся по ссылке')
  const p = panel()
  t.ok('питерский домен', p.q('shopFromUrl("https://petrovich.ru/product/123")') === 'Петрович')
  t.ok('московский поддомен', p.q('shopFromUrl("https://moscow.petrovich.ru/product/123")') === 'Петрович')
  t.ok('www тоже', p.q('shopFromUrl("https://www.petrovich.ru/product/123")') === 'Петрович')
  t.ok('домен-двойник не путаем', p.q('shopFromUrl("https://petrovich.com/x")') === 'petrovich.com',
    'чужой домен принят за Петровича')
  t.ok('он есть в ряду «сравнить»',
    p.q('PRICE_SHOPS.map(function(x){return x.n;}).join(",")').indexOf('Петрович') >= 0)
  t.ok('и ищет по региональному адресу',
    /moscow\.petrovich\.ru/.test(p.q('(PRICE_SHOPS.filter(function(x){return x.n==="Петрович";})[0]||{}).q')),
    'безрегиональный адрес отдаст цену чужого города')

  // Цвет магазина один на весь портал: в снабжении своя карта, и разойдись они —
  // один и тот же Петрович будет разного цвета на соседних экранах.
  t.ok('цвет совпадает со сверкой',
    p.q('SC["Петрович"]') === '#e8552d', String(p.q('SC["Петрович"]')))
}
